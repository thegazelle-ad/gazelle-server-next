...
const Featured = ({ article }: { article: ArticlePreview }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4 md:gap-4 w-full">
        {/* Image */}
        <Link href={getArticleUrl(article)} className="relative min-h-[350px] md:min-h-[350px] lg:min-h-[400px] h-auto w-full md:col-span-2">
          <Image 
            src={article.image}
            alt={article.title} 
            priority={true} 
        ...
        {/* Title and info */}
        <div className="flex flex-col justify-center gap-4 md:gap-2 w-full">
          {/* Category */}
          <Link href={`/category/${article.category.slug}`} >
            <p className="text-xl md:text-lg capitalize text-gray-600 font-semibold">{article.category.name}</p>
          </Link>
          {/* Title */}
